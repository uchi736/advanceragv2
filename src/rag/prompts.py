...
元の質問と専門用語定義を活用して、RAG検索エンジンで最大限の関連情報を取得できる質問に再構築してください。

【改良方針】
1. **質問意図の保持**: 元の質問の核心的な意図を変えない
2. **定義の自然な統合**: 専門用語定義から得られる関連概念・類義語・技術要素を質問文中に自然に組み込む（定義文をそのまま貼り付けない）
3. **検索語の追加**: ドキュメントに出現する可能性の高い関連キーワードを1-3語程度追加
4. **簡潔さと明確さ**: 50-100文字程度、冗長な説明的語句や重複表現は避ける
